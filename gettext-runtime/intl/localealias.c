...
static size_t internal_function read_alias_file(const char *fname,
                                                int fname_len) {
  FILE *fp;
  char *full_fname;
  size_t added;
    ...
    if (cp[0] != '\0' && cp[0] != '#') {
      ...
      if (cp[0] != '\0') {
...
#endif
        {
          ...
          map[nmap].alias = (const char *)memcpy(
              &string_space[string_space_act], alias, alias_len);
          string_space_act += alias_len;

          map[nmap].value = (const char *)memcpy(
              &string_space[string_space_act], value, value_len);
